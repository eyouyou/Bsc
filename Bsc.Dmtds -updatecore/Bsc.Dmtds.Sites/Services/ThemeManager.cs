...
namespace Bsc.Dmtds.Sites.Services
{
    ...
    [Dependency(typeof(FileManager), Key = "themes")]
    public class ThemeManager: FileManager
    {
        ...
        private IEnumerable<string> GetCssExtensions()
        {
            List<string> list = new List<string>();
            list.Add(".css");
            var dynamicClientResource = EngineContext.Current.ResolveAll<Bsc.Dmtds.Core.Mvc.WebResourceLoader.DynamicClientResource.IDynamicClientResource>().Where(it => it.ResourceType == Bsc.Dmtds.Core.Mvc.WebResourceLoader.DynamicClientResource.ResourceType.Stylesheet);
