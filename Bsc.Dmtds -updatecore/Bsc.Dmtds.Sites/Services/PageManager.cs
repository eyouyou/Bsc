...
using Bsc.Dmtds.Search;
using Bsc.Dmtds.Sites.Caching;
using Bsc.Dmtds.Sites.Models;
using Bsc.Dmtds.Sites.Persistence;
using Bsc.Dmtds.Sites.Persistence.Caching;
using Bsc.Dmtds.Sites.Versioning;
using SiteMap = Bsc.Dmtds.Sites.Models.SiteMap;
using SiteMapNode = Bsc.Dmtds.Sites.Models.SiteMapNode;
...
namespace Bsc.Dmtds.Sites.Services
{
    public class PageManager: PathResourceManagerBase<Page, IPageProvider>
    {
        ...
        public virtual Page Copy(Site site, string sourcePageFullName, string destPageFullName)
        {
            ...
            if (destPage.Exists())
            {
                throw new BscException("The page already exists.");
            }
        ...
        public virtual void Move(Site site, string pageFullName, string newParent, bool createRedirect, string user = "")
        {
            ...
            if (string.IsNullOrEmpty(newParent))
            {
                if (page.Parent == null)
                {
                    throw new BscException("The page is a root page already.");
                }
            }
        ...
        {
            string urlPath = UrlUtility.UrlSeparatorChar + UrlUtility.Combine(pagePaths);
