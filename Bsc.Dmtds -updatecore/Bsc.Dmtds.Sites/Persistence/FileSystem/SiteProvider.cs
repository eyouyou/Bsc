...
using Bsc.Dmtds.Membership.Persistence;
using Bsc.Dmtds.Sites.Models;
using Bsc.Dmtds.Sites.Models.Options;
using Ionic.Zip;

namespace Bsc.Dmtds.Sites.Persistence.FileSystem
{
    [Bsc.Dmtds.Core.Runtime.Dependency.Dependency(typeof(ISiteProvider))]
    [Bsc.Dmtds.Core.Runtime.Dependency.Dependency(typeof(IProvider<Site>))]
    public class SiteProvider : ISiteProvider
    {
        ...
        private void ClearSiteData(Site site)
        {
            ...
            try
            {
                Bsc.Dmtds.Sites.Persistence.Providers.PageProvider.Clear(site);
            }
            catch
            {
            }
            try
            {
                Bsc.Dmtds.Sites.Persistence.Providers.HtmlBlockProvider.Clear(site);
            }
            catch
            {
            }
        ...
        public virtual IEnumerable<Site> ChildSites(Site site)
        {
            ...
            {
                foreach (var dir in Bsc.Dmtds.Common.IO.IOUtility.EnumerateDirectoriesExludeHidden(baseDir))
                {
        ...
        public virtual void Offline(Site site)
        {
            ...
            {
                File.WriteAllText(offlineFile, "The site is offline, please remove this file to take it online.");
            }
        }
        ...
        public virtual Site Create(Site parentSite, string siteName, System.IO.Stream packageStream, CreateSiteOptions options)
        {
            ...
            {
                Bsc.Dmtds.Common.HealthMonitoring.Log.LogException(ex);

                Bsc.Dmtds.Common.IO.IOUtility.DeleteDirectory(site.PhysicalPath, true);
        ...
        private void CreateSubSite(Site parentSite, string siteName, CreateSiteOptions options)
        {
            if (parentSite == null)
            {
                throw new Exception("Parent site is required when create a sub site.");
            }
        ...
        private void CopyRepository(Bsc.Dmtds.Content.Models.Repository sourceRepository, string name)
        {
            var repositoryManager = Bsc.Dmtds.Content.Services.ServiceFactory.RepositoryManager;
        ...
        {
            string sitesBaseVirtualPath = "/" + baseDir.DataPathName + "/Sites";
            string siteFilePathPattern = sitesBaseVirtualPath + "/[^/]+/";
            string siteFileReplacement = sitesBaseVirtualPath + "/" + (site.Name ?? "") + "/";

            foreach (var file in Bsc.Dmtds.Common.IO.IOUtility.EnumerateFiles(site.PhysicalPath, new[] { "*.cshtml", "*.html", "*.xml" }, SearchOption.AllDirectories))
            {
        ...
        private void CreateMembership(Site site, CreateSiteOptions options)
        {
            ...
            {
                var membership = new Bsc.Dmtds.Membership.Models.Membership(membershipName).AsActual();
                if (membership == null)
                {
        ...
        private void ExportSiteRepository(Site rootSiteExported, Site site, ZipFile zipFile, bool includeSubSites)
        {
            ...
            if (!string.IsNullOrEmpty(site.Repository))
            {
                    ...
                    && !Get(site.Parent).Repository.EqualsOrNullEmpty(site.Repository, StringComparison.CurrentCultureIgnoreCase)))
                {
                    ...
                    Bsc.Dmtds.Content.Services.ServiceFactory.RepositoryManager.Export(site.Repository, ms);
