...
namespace Bsc.Dmtds.Sites.View.WebProxy
{
    ...
    {
        private IProxyHtmlParser _htmlParser;
        private IHttpProcessor _httpProcessor;
        ...
        {
            Func<string, bool, string> proxyUrl =
                (url, isForm) => new Uri(proxyRenderContext.ProxyPosition.HostUri, url).ToString();
            ...
            var html = _httpProcessor.ProcessRequest(proxyRenderContext.ControllerContext.HttpContext,
                proxyRenderContext.RequestUri.ToString(), proxyRenderContext.HttpMethod, proxyUrl);
        ...
        protected string GenerateProxyUrl(ProxyRenderContext proxyRenderContext, string url, bool isForm)
        {
                ...
                !Uri.IsWellFormedUriString(url, UriKind.Absolute))
            {
                ...
                var routeValues =
                    proxyRenderContext.PageRequestContext.ModuleUrlContext.GetRouteValuesWithModuleUrl(
                ...
                var pageUrl =
                    urlHelper.FrontUrl(proxyRenderContext.PageRequestContext.Site,
                        proxyRenderContext.PageRequestContext.RequestChannel)
                        .PageUrl(proxyRenderContext.PageRequestContext.Page.FullName, routeValues)
                        .ToString();
                if (isForm)
                {
                    pageUrl = UrlUtility.AddQueryParam(pageUrl,
                        View.ModuleUrlContext.PostModuleParameter,
                        proxyRenderContext.ProxyPosition.PagePositionId);
                }
