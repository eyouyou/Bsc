...
namespace Bsc.Dmtds.Search
{
    ...
    public class SearchService : ISearchService
    {
        ...
        {
            var writer = new IndexWriter(FSDirectory.Open(new DirectoryInfo(indexDir)), Analyzer,
            ...
            Thread thread = new Thread(() =>
            {
                try
                {
                    if (o is TextContent)
                    {
                        ...
                        EngineContext.Current.Resolve<Bsc.Dmtds.Search.Persistence.ILastActionProvider>()
                            .Add(lastAction);
                    }
                }
                catch (Exception e)
        ...
        public virtual PagedList<Models.ResultObject> Search(string key, int pageIndex, int pageSize,
            params string[] folders)
        {
            var indexDirectory = FSDirectory.Open(new DirectoryInfo(indexDir));
            if (!IndexReader.IndexExists(indexDirectory) ||
                string.IsNullOrEmpty(key) && (folders == null || folders.Length == 0))
            {
            ...
            QueryParser titleParser = new QueryParser(Lucene.Net.Util.Version.LUCENE_30, Converter.TitleFieldName,
                this.Analyzer);
            var titleQuery = titleParser.Parse(key);
            titleQuery.Boost = 2;
            query.Add(new BooleanClause(titleQuery, Occur.SHOULD));

            QueryParser bodyParser = new QueryParser(Lucene.Net.Util.Version.LUCENE_30, Converter.BodyFieldName,
                this.Analyzer);
            ...
            var lighter =
                new Highlighter(new SimpleHTMLFormatter("<strong class='highlight'>", "</strong>"),
                    new Lucene.Net.Search.Highlight.QueryScorer((Query) query));
