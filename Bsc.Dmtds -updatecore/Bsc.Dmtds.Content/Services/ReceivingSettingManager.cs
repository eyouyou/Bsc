            ...
            else
            {
                foreach (var drivedContent in contents)
                {
                    if (drivedContent.IsLocalized != null && drivedContent.IsLocalized.Value == false)
                    {
                        ...
                        var versions = Versioning.VersionManager.AllVersionInfos(originalContent);
                        if (versions.Count() > 0)
                        {
                            drivedContent.OriginalLastestVisitedVersionId = Versioning.VersionManager.AllVersionInfos(originalContent).Max(it => it.Version);
                        }
