...
namespace Bsc.Dmtds.Core.Mvc.Grid
{
    public class GridModel : IGridModel
    {
        ...
        public static IGridModel CreateGridModel(Type modelType, IEnumerable dataSource, ViewContext viewContext)
        {
            ...
            if (!(modelMetadata is BscModelMetadata))
            {
                throw new InvalidOperationException("请安装。。。, etc:ModelMetadataProviders.Current = new Kooboo.Web.Mvc.KoobooDataAnnotationsModelMetadataProvider()");
            }
            var gridAttribute = ((BscModelMetadata)modelMetadata).Attributes.OfType<GridAttribute>().FirstOrDefault();
        ...
        {
            foreach (var att in ((BscModelMetadata)propertyMetadata).Attributes.OfType<GridColumnAttribute>())
            {
